...
export default function SuppliersUpdateForm() {
  const router = useRouter();
  const supplierId = router.query.id as string;

  const updateSupplier = api.supplier.update.useMutation();
  const supplierInfo = api.supplier.getById.useQuery({
  ...
  const form = useForm<z.infer<typeof suppliersFormSchema>>({
    ...
    defaultValues: {
      email: supplierInfo.data?.email ?? undefined,
      location: supplierInfo.data?.location ?? undefined,
      name: supplierInfo.data?.name ?? "",
      phoneNumber: supplierInfo.data?.phoneNumber ?? undefined,
      site: supplierInfo.data?.site ?? undefined,
    },
  });

  useEffect(() => {
    if (supplierInfo.data) {
      const { email, location, name, phoneNumber, site } = supplierInfo.data;
      form.reset({
      ...
      <div className="hidden space-y-6 p-10 pb-16 md:block">
        <div className="space-y-0.5">
          ...
          <p className="text-muted-foreground">
            Actualizar dados de um fornecedor
          </p>
        </div>
            ...
            <div className="space-y-6">
              <Form {...form}>
                ...
                >
                  ...
                  <Button type="submit">Actualizar Fornecedor</Button>
                </form>
              </Form>
            </div>
          </div>
